...
All notable changes to **novacat** will be documented in this file.
...
- Added `session_timeout` to `FastAPIWebsocketTransport` and `WebsocketServerTransport`
  ...
  See [examples/websocket-server/bot.py](https://github.com/novacat-ai/novacat/blob/main/examples/websocket-server/bot.py).
...
- Changed `InputParams` in `src/novacat/services/gemini_multimodal_live/gemini.py` to support different modalities.
...
- novacat now supports Python 3.13. We had a dependency on the `audioop` package
...
- Fixed an issue that would cause an import error when importing
  `SileroVADAnalyzer` from the old package `novacat.vad.silero`.
...
- novacat has a pipeline-based architecture. The pipeline consists of frame
...
- novacat now uses Ruff as its formatter (https://github.com/astral-sh/ruff).
...
- `TwilioFrameSerializer` can now specify Twilio's and novacat's desired sample
  rates to use.
...
- Added `twilio-chatbot`. This is an example that shows how to integrate Twilio
  phone numbers with a novacat bot.
...
- Moved `ParallelTask` to `novacat.pipeline.parallel_task`.
